...
impl BlockDevice for NVMeBlock {
    fn read_block(&self, block_id: usize, buf: &mut [u8]) {
        ...
        if nb {
            ...
            self.nvme_blk.exclusive_session(|nvme| {
                let ns = nvme.get_ns(1).expect("Namespace ID 1 not found");
                nvme.send_io_read(&ns, block_id as u64, 1 as u16);
            });
    ...
    fn write_block(&self, block_id: usize, buf: &[u8]) {
        ...
        if nb {
            ...
            self.nvme_blk.exclusive_session(|nvme| {
                let ns = nvme.get_ns(1).expect("Namespace ID 1 not found");
                nvme.send_io_write(&ns, block_id as u64, 1 as u16, dma_buf.paddr().0);
            });
            let task_cx_ptr = self.condvar.wait_no_sched();
            crate::task::schedule(task_cx_ptr);
            // TODO: currently handler ACK completion, so we just retrieve data here. This has to be done here
    ...
    fn handle_irq(&self) {
        ...
        if self.doing_io.exclusive_access().clone() == false {
            self.nvme_blk.exclusive_session(|nvme| {
                let io_sq = &nvme.get_io_submission_queue();
                let sq_peek = io_sq.peek();
                log::warn!("IO Submission Queue Peek: {:?}", sq_peek);
            });
            return;
        }
        self.nvme_blk.exclusive_session(|nvme| {
