...
impl BlockDevice for NVMeBlock {
    fn read_block(&self, block_id: usize, buf: &mut [u8]) {
        ...
        log::info!("NVMe read_block {} requested for block {}", if nb { "async" } else { "sync" }, block_id);
        if nb {
    ...
    fn write_block(&self, block_id: usize, buf: &[u8]) {
        let nb = false;
        // let nb = *DEV_NON_BLOCKING_ACCESS.exclusive_access();
        log::info!("NVMe write_block {} requested for block {}", if nb { "async" } else { "sync" }, block_id);
        // create dma buffer and copy data
        let mut dma_buf = Dma::<[u8; 512]>::new().unwrap();
    ...
    fn handle_irq(&self) {
        ...
        self.nvme_blk.exclusive_session(|nvme| {
            if let Some(_) = nvme.io_peek_command() {
                self.condvar.signal();
            } else {
                log::warn!("NVMe IRQ received but no completion entry found");
                return;
            }
        });
    }
